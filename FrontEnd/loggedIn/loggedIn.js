...
invitebtn.addEventListener("click", async (e) => {
  e.preventDefault();
  try {
    await axios.get(`${inviteLink.value}`, {
...
crtGrp.addEventListener("click", async (e) => {
  ...
  if (grpName) {
    try {
      await axios.get(`http://localhost:3000/groupParams/${grpName}`, {
        headers: { Authorization: localStorage.getItem("token") },
      });
    } catch (err) {
      console.log(err);
...
export async function renderChat(groupName, groupId) {
    ...
    if (e.target.textContent == "Copy invite link") {
      ...
      try {
        const result = await axios.get(
          `http://localhost:3000/groupInfo/${groupId}`,
          { headers: { Authorization: localStorage.getItem("token") } }
        );
        chattbody.innerHTML = ``;
        result.data
          ...
          .forEach(async (element) => {
            ...
            if (result.data.slice(-1)[0] === true) {
              if (element.member == element.admin) {
                data.appendChild(
                  document.createTextNode(`${User.data.name}(ADMIN)`)
                );
                ...
                deleteb.addEventListener("click", async (e) => {
                  e.preventDefault();
                  await axios.get(
                    `http://localhost:3000/removeMember?memberId=${e.target.parentElement.id}&groupId=${groupId}`,
                    {
                      headers: { Authorization: localStorage.getItem("token") },
                    }
                  );
      ...
      try {
        await axios.get(`http://localhost:3000/deleteGroup/${groupId}`, {
          headers: { Authorization: localStorage.getItem("token") },
        });
        location.reload();
      } catch (err) {
  ...
  try {
    const chatData = await axios.get(
      `http://localhost:3000/getChat/?MessageId=${
        JSON.parse(localStorage.getItem("message"))[-1]
      }&GroupId=${groupId}`,
      { headers: { Authorization: localStorage.getItem("token") } }
    );
    if (chatData) {
    ...
    for (let element of array) {
      if (element.chat.indexOf(".txt") == -1) {
        ...
        try {
          ...
          data.appendChild(
            document.createTextNode(`${User.data.name}: ` + msg)
          );
          row.appendChild(data);
          chattbody.appendChild(row);
        } catch (error) {
        ...
        const base64 = await axios.get(
          `http://localhost:3000/fetchbase64/${element.chat}`,
          { headers: { Authorization: localStorage.getItem("token") } }
        );
  ...
  send.addEventListener("click", async (e) => {
    ...
    try {
      ...
      try {
        ...
        data.appendChild(
          document.createTextNode(`${User.data.name}: ` + result.data.chat)
        );
        row.appendChild(data);
        chattbody.appendChild(row);
      } catch (err) {
...
async function renderGroup() {
  try {
    const result = await axios.get(`http://localhost:3000/group/getGroup`, {
      headers: { Authorization: localStorage.getItem("token") },
    });
    console.log(">>>>>>", result);
